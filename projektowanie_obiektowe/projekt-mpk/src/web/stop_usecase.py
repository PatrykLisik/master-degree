...
async def get_all_stop_usecase(stop_repository: AbstractStopRepository) -> set[Stop]:
    return await stop_repository.get_all()


async def get_many_stop_usecase(stop_repository: AbstractStopRepository, stops_ids: set[str]) -> set[Stop]:
    return await stop_repository.get_many(stops_ids)
