...
async def add_stop_usecase(
    stop_repository: AbstractStopRepository, name, geo_x, geo_y
) -> Stop:
    return await stop_repository.add(
        name=name, geolocation_x=geo_x, geolocation_y=geo_y
    )


async def get_stop_usecase(
    stop_repository: AbstractStopRepository, stop_id: str
) -> Stop:
...
async def get_many_stop_usecase(
    stop_repository: AbstractStopRepository, stops_ids: set[str]
) -> list[Stop]:
    return [stop for stop in await stop_repository.get_many(stops_ids)]


async def add_time_between_stops_usecase(
