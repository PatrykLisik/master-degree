...
from src.web.api.transit_usecase import (
    add_transit_usecase,
    get_all_transits_usecase,
    get_transit_usecase,
)
...
async def add_transit(
    request, transit_repository: AbstractTransitRepository
) -> HTTPResponse:
    request_data = request.json
    transit = await add_transit_usecase(
...
async def get_transit(
    request, transit_id, transit_repository: AbstractTransitRepository
) -> HTTPResponse:
    transit = await get_transit_usecase(
        transit_repository=transit_repository, transit_id=transit_id
    )
