...
async def add_driver_usecase(
    driver_repository: AbstractDriverRepository,
    name: str,
    surname: str,
    pesel: str,
    phone: str,
) -> Driver:
    driver = await driver_repository.add(
        first_name=name, last_name=surname, pesel=pesel, phone=phone
    )
    return driver


async def get_driver_usecase(
    driver_repository: AbstractDriverRepository, driver_id
) -> Driver:
    return await driver_repository.get(driver_id)


async def get_all_drivers_usecase(
    driver_repository: AbstractDriverRepository,
) -> List[Driver]:
    return [driver for driver in await driver_repository.get_all()]


async def update_driver_use_case(
    driver_repository: AbstractDriverRepository,
    name: Optional[str],
