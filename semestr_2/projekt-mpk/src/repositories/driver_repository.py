...
class InMemoryDriverRepository(AbstractDriverRepository):
    ...
    async def add(
        ...
        return await infile_driver_to_domain(new_driver)
    ...
    async def get(self, driver_id: str) -> Optional[DomainDriver]:
        ...
        if infile_driver is None:
            return None
        return await infile_driver_to_domain(infile_driver)

    async def get_all(self) -> Set[DomainDriver]:
        return {
            await infile_driver_to_domain(infile_driver)
            for infile_driver in self._data.values()
        }
...
class InFileDriverRepository(AbstractDriverRepository):
    ...
    def _get(self) -> Dict[str, InFileDriver]:
        try:
            with open(
                ...
                return {
                    driver_data["id"]: InFileDriver(**driver_data)
    ...
    async def get_all(self) -> Set[DomainDriver]:
        return {
            await infile_driver_to_domain(infile_driver)
            for infile_driver in self._get().values()
        }
    ...
    async def add(
        ...
        async with self.session_maker() as session:
            async with session.begin():
                new_driver = DBDriver(
                    first_name=first_name, last_name=last_name, pesel=pesel, phone=phone
                )
